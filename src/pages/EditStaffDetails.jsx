...
export default function EditStaffDetails() {
    ...
    const handleSubmit = async (values) => {
        try {
            const response = await fetch(`https://localhost:7265/api/Staff/UpdateStaff`, {
                method: 'PUT',
                headers: {
            ...
            setSuccessDialogMessage('Staff details updated successfully.');
            onSuccessDialogOpen();
        } catch (error) {
            setDialogMessage(error.message || 'Failed to update staff details.');
            onDialogOpen();
        }
    };
    ...
    return (
        <>
            <PageHeader title="Edit Staff Details" breadcrumbs={breadcrumbs} />
            <Formik
                enableReinitialize
